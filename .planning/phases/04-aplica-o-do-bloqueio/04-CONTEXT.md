...
- **D-07 (corrigido por `04-RESEARCH.md` — Achado crítico 1):** São **6** caminhos de leitura pública, não 5. Ao mapa original (página+layout do catálogo, `GET /api/catalogo/produtos`, `GET /api/catalogo/produtos/[id]`, `GET /api/catalogo/combos`, `GET /api/catalogo/combos/[id]`) soma-se `GET /api/empresas/slug/[slug]` — endpoint totalmente público, sem `requireAuth`, que devolve produtos+combos+promoções inteiros via `empresaService.findBySlug`. Já estava mapeado em `.planning/research/PITFALLS.md` de milestone anterior mas não sobreviveu até esta discussão. Todos os 6 tratam uma empresa bloqueada **exatamente como uma empresa/slug/id inexistente** — mesmo código de resposta, mesmo formato de corpo, nenhuma menção a pagamento. Isso vale inclusive para os endpoints que aceitam `empresaId` diretamente por query string — não é permitido vazar para um visitante externo que aquela empresa existe mas está com o pagamento atrasado.
- **D-07b (achado da pesquisa, não uma nova decisão de produto):** `getEmpresaCatalogo` — a função por trás da página do catálogo — também é consumida por `app/[slug]/admin/login/page.tsx` só para pegar logo/cores. Se o gate de bloqueio entrasse ali dentro, a tela de login de uma empresa bloqueada viraria 404 e ninguém conseguiria logar para pagar (quebra ACC-04/D-04). A solução (separar uma leitura de "branding" sem gate de uma leitura "publicável" com gate) é implementação, não decisão de produto — mas registrada aqui porque quase inverteu o objetivo da fase.
...
### Oráculo residual da tela de login (achado da pesquisa, decisão confirmada pelo usuário)
- **D-09:** A tela de login do admin (`/[slug]/admin/login`) precisa continuar respondendo 200 para uma empresa bloqueada — senão o usuário nunca consegue logar para pagar (quebraria ACC-04/D-04). Isso cria um vazamento residual: um visitante externo consegue diferenciar "slug nunca existiu" (404 em `/{slug}` e em `/{slug}/admin/login`) de "existe mas está bloqueada" (404 no catálogo, 200 no login). Mitigação escolhida: quando a empresa está bloqueada, a tela de login renderiza com **branding genérico** (sem nome, logo ou cores da loja — visual padrão do sistema), em vez do branding real. Reduz o vazamento ao mínimo possível: revela que existe uma porta de admin naquele slug, mas não a identidade/marca do negócio.
